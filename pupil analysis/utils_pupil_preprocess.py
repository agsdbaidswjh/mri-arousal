...
def regress_blinks(df, events, params, interval=7, regress_blinks=True, regress_sacs=True):
    ...
    # params:
    x = np.linspace(0, interval, int(interval * params['fs']), endpoint=False)
    ...
    blink_ends = (events.loc[events['description']=='BAD_blink', 'onset'].values +
                  events.loc[events['description']=='BAD_blink', 'duration'].values)
...
def preprocess_pupil(samples, events, params, df_bw=None):
    ...
    regress_blinks(df=samples, events=events, params=params, interval=7, regress_blinks=True, regress_sacs=True)
